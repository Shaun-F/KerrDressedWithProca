...
BeginPackage["QNMovertoneSearch`"]

QNMoverSearchPrec::usage "QNMoverSearch[m,n,spin,\[Mu]start,\[Mu]stop,Initial branch,Im(\[Omega])]"

Begin["QNMmode`"]


QNMoverSearchPrec[min_,nin_,spin_,\[Mu]sta_,\[Mu]sto_,branchk_,\[Omega]ImIn_,Sin_,direc_,wroffset_]:=Module[{min1=min,nin1=nin,spin1=spin,\[Mu]sta1=\[Mu]sta,\[Mu]sto1=\[Mu]sto,brach1=branchk,wIKOK=\[Omega]ImIn},
...
Print["Considering Parameters:"];
Print["m = "<>ToString[m]];
Print["n = "<>ToString[nh]];
Print["\[Chi] = "<>ToString[\[Chi]//N]];
Print["\[Mu]start = "<>ToString[\[Mu]sta]];
Print["S = "<>ToString[Sin//N]];
Print["branch = "<>ToString[branchk]];
Print["Precision cutoff = "<>ToString[preccutoff]];
Print["rplus+\[Epsilon] = rplus + "<>ToString[\[Epsilon]]];
Print["-------------------------------------"];
...
rstop[\[Mu]_]:=If[min<2,(35(m+nh))/\[Mu]^2//Rationalize[#,0]&,If[min<4,(50(m+nh))/\[Mu]^2//Rationalize[#,0]&,(60(m+nh))/\[Mu]^2//Rationalize[#,2]&]];
...
DiffRadx=1/(x (1+x)) (-(\[Mu]n^2/\[Nu]n^2)-\[Mu]n^2 (1+Sqrt[1-\[Chi]^2]+2 x Sqrt[1-\[Chi]^2])^2-(2 M \[Nu]n (1+Sqrt[1-\[Chi]^2]+2 x Sqrt[1-\[Chi]^2]) (1-\[Chi]^2+Sqrt[1-\[Chi]^2]-2 x (-1+\[Chi]^2)) (m \[Nu]n^2 \[Chi]-I (-1+\[Nu]n^2 \[Chi]^2) (\[Omega]ni-I \[Omega]nr)))/(Sqrt[1-\[Chi]^2] (M+M \[Nu]n^2 (2-\[Chi]^2+2 Sqrt[1-\[Chi]^2]-4 x^2 (-1+\[Chi]^2)+4 x (1-\[Chi]^2+Sqrt[1-\[Chi]^2]))))+(I m \[Chi]+2 (1+Sqrt[1-\[Chi]^2]-2 x^2 (-1+\[Chi]^2)+2 x (1-\[Chi]^2+Sqrt[1-\[Chi]^2])) (\[Omega]ni-I \[Omega]nr))^2/(4 x (1+x) (-1+\[Chi]^2))+(I \[Omega]ni+\[Omega]nr)/\[Nu]n+\[Nu]n \[Chi] (m-\[Chi] (I \[Omega]ni+\[Omega]nr))) #+(1+2 x+(4 M x (1+x) \[Nu]n^2 (-1+\[Chi]^2) (1+Sqrt[1-\[Chi]^2]+2 x Sqrt[1-\[Chi]^2]))/(Sqrt[1-\[Chi]^2] (M+M \[Nu]n^2 (2-\[Chi]^2+2 Sqrt[1-\[Chi]^2]-4 x^2 (-1+\[Chi]^2)+4 x (1-\[Chi]^2+Sqrt[1-\[Chi]^2]))))) /(x (1+x)) D[#,x]+D[#,{x,2}]&;
	...
	(*Precision minimization function*)
	Rmaxfunc[wIn_,\[Nu]In_,\[Mu]In_,branch2_]:=Module[{wIn1=wIn,\[Nu]In1=\[Nu]In,\[Mu]In1=\[Mu]In,branch14=branch2},
	\[Mu]i=SetPrecision[\[Mu]In,setprec];
	...
	Frob[x_]:=x^(-I \[Lambda]) (1+Sum[Symbol["QNMmode`c"<>ToString[n]]x^n,{n,1,2}]);
	...
	Coeff=NSolve[exptemp1,{\[Lambda],c1,c2},WorkingPrecision->setprec-2(*setprecNSolve[i]*)][[branch2]];
	...
	FrobOfr[rtt_]:=SetPrecision[Frob[x]//.Coeff//.{x->(rtt M-rp[a])/(rp[a]-rm[a])}(*/.{\[Lambda]\[Rule]\[CapitalKappa]}*)/.{\[Mu]n->\[Mu]i,\[Omega]nr->wrpoint,\[Omega]ni->wipoint}//Simplify,setprec];
	R0=SetPrecision[Limit[FrobOfr[rt],rt->rtstart[a]],setprec];
	dR0=SetPrecision[Limit[D[FrobOfr[rt],rt],rt->rtstart[a]],setprec];
		
	solR=NDSolve[{(DiffRadrt@R[rt]/.{\[Nu]n->r\[Nu]+I i\[Nu]}/.\[Nu]root/.{\[Mu]n->\[Mu]i,\[Omega]nr->wrpoint,\[Omega]ni->wipoint})==0,R[rtstart[a]]==R0,R'[rtstart[a]]==dR0},R,{rt,rtstart[a],rstop[\[Mu]i]},Method->{"StiffnessSwitching"},WorkingPrecision->setprec-2,MaxSteps->maxstep,AccuracyGoal->5]//First;
...
funcmin[win_,\[Nu]in_,\[Mu]in_,branch1_]:=Module[{win1=win,\[Nu]in1=\[Nu]in,\[Mu]in1=\[Mu]in},
Rmaxfunc[win,\[Nu]in,\[Mu]in,branch1];
Rmax
];
...
\[Omega]nonRel[\[Mu]n_,na_]:=\[Mu]n(1-\[Mu]n^2/(2na^2))-wroffset//Rationalize[#,0]&;(*Provides only a guess for the real part.*)
If[Sin==-1,
(*This is for S=-1*)
Print["Picked S=-1 \[Nu]nonrel"];
\[Nu]nonRel[\[Omega]a_]:=-\[Omega]a/(m-\[Chi] \[Omega]a)+0.01//Rationalize[#,0]&; 
,
If[Sin==0,
(*This is for S=0*)
Print["Picked S=0 \[Nu]nonrel"];
\[Nu]nonRel[\[Omega]a_]:=1/(2\[Chi])(min+1-\[Chi] \[Omega]a+Sqrt[(\[Chi] \[Omega]a-min-1)^2+4\[Chi] \[Omega]a]);
,
If[Sin==1, (*This is for S=+1*)
Print["Picked S=+1 \[Nu]nonrel"];
\[Nu]nonRel[\[Omega]b_]:=v//.(NSolve[a v^3(min-a \[Omega]a)-((min+1)(min+2)-a \[Omega]a(2 min-a \[Omega]a))v^2+\[Omega]a v+\[Omega]a^2,v][[2]]//Simplify)/.{M->1}//.{\[Omega]a->\[Omega]b};
,
...
If[direc==+1,
Print["Direction of evolution: "<>ToString[direc]];
If[\[Mu]start>\[Mu]stop,Print["Error: \[Mu] bounds don't match direction of evolution"];Abort[],{}];
\[CapitalDelta]\[Mu][\[Mu]n_,na_]:=3/4 Min[Abs[\[Omega]nonRel[\[Mu]n,na]-\[Omega]nonRel[\[Mu]n,na+1]],Abs[\[Omega]nonRel[\[Mu]n,na]-\[Omega]nonRel[\[Mu]n,na-1]]];
\[Mu]0=\[Mu]start;
\[Mu]r[1]=\[Mu]start;
i=2;
	While[\[Mu]0<\[Mu]stop,
	\[Mu]r[i]=\[Mu]0+If[\[CapitalDelta]\[Mu][\[Mu]0,n]>Abs[\[Mu]stop-\[Mu]start]/100,Abs[\[Mu]stop-\[Mu]start]/99,\[CapitalDelta]\[Mu][\[Mu]0,n]];
	\[Mu]0=\[Mu]r[i];
	;i++];
\[Mu]range={0,Table[\[Mu]r[k],{k,1,i-1}]//Rationalize[#,0]&}//Flatten;
...
i=2;
        While[\[Mu]0>\[Mu]stop,
        \[Mu]r[i]=\[Mu]0-If[\[CapitalDelta]\[Mu][\[Mu]0,n]>Abs[\[Mu]stop-\[Mu]start]/100,Abs[\[Mu]stop-\[Mu]start]/80,\[CapitalDelta]\[Mu][\[Mu]0,n]];
        \[Mu]0=\[Mu]r[i];
        ;i++];
\[Mu]range={0,Table[\[Mu]r[k],{k,1,i-1}]//Rationalize[#,0]&}//Flatten;
\[Mu]all=Take[\[Mu]range,-Length[\[Mu]range]+1];
...
plotlist=ConstantArray[0,{Length[\[Mu]range],\[Omega]iprecstep}];
...
iiinitial=1;
iidelta[u_]:=SetPrecision[If[u<4||m==5||m==4||m==3||m==2,iiinitial,If[Abs[Log10[Im[\[Omega]minall[[u]]]]-Log10[Im[\[Omega]minall[[u-1]]]]]>iiThreshhold,Abs[Log10[Im[\[Omega]minall[[u]]]]-Log10[Im[\[Omega]minall[[u-1]]]]],1]],setprec];

(*The code:*)
\[Delta]\[Omega]r=0;
offset=0;
\[Nu]root=\[Nu]rootminall[[1]];
Clear[y];
Do[
Print["-----------------------------------"];
Print[ToString[i]<>"/"<>ToString[\[Mu]rangenumber]];
\[Mu]i=\[Mu]range[[i+1]];
Print[\[Mu]i//N];
...
wintr=SetPrecision[Re[\[Omega]minall[[i]]],setprec];
...
wrpoint=wintr;
\[Delta]\[Omega]r=10rrdelta[i]/\[Omega]rstepsize;
	For[y=1,y<=\[Omega]rprecstep,y++,
	aa=1;
	aatest=-1;
	While[aatest<0,
			...
			\[Nu]rpoint=r\[Nu]/.\[Nu]root;
			\[Nu]ipoint=i\[Nu]/.\[Nu]root;
			...
			exptemp=Series[SetPrecision[x^(I \[Lambda] +2) (DiffRadx@Frob[x])//Simplify,setprec],{x,0,2}]==0;
			exptemp1=SetPrecision[exptemp/.{\[Mu]n->\[Mu]i}/.{\[Omega]nr->wrpoint,\[Omega]ni->wipoint}/.{\[Nu]n->r\[Nu]+I i\[Nu]}/.\[Nu]root//LogicalExpand,setprec];
			Coeff=NSolve[exptemp1,{\[Lambda],c1,c2},WorkingPrecision->setprec-2][[l]];
			...
			\[Omega]IterationR[aa]=wrpoint;

			maxR[aa]=SetPrecision[Log[Abs[R[rt]/.solRIterationR[aa]/.{rt->rstop[\[Mu]i]}]],setprec];
			aatest=If[aa<=2,-1,maxR[aa]-maxR[aa-1]];
			If[aa>500,Print["Terminated loop 1"];Break[];,{}];
		;aa++];
		\[Omega]minR=\[Omega]IterationR[aa-1];
		wintr=\[Omega]minR;
		rrboundary=Abs[\[Omega]IterationR[aa-2]-\[Omega]IterationR[aa-1]];
		\[Delta]\[Omega]r=5rrboundary / \[Omega]rstepsize;
		If[aa>500,Break[];,{}];
...
(*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*)
			Clear[y];
			count=0;			
			For[y=1,y<=\[Omega]iprecstep,y++,
			\[Nu]rootIterationI=ConstantArray[0,{\[Omega]mesh}];
			solRIterationI=ConstantArray[0,{\[Omega]mesh}];
			\[Omega]IterationI=ConstantArray[0,{\[Omega]mesh}];
				Do[
				wrpoint=SetPrecision[\[Omega]minR,setprec];
				wipoint=SetPrecision[\[Omega]irange[[bb]],setprec];
			
				(*To pick the correct \[Nu] solution form the set of roots of the EVP, we compare it to either the non-rel. limit using the current \[Omega], or the previous (previous mass) minimal result for \[Nu]*)
				\[Nu]rpoint=r\[Nu]/.\[Nu]root;
				\[Nu]ipoint=i\[Nu]/.\[Nu]root;				
				(*\[Nu]rpoint=r\[Nu]/.\[Nu]rootminall[[i]];
				\[Nu]ipoint=i\[Nu]/.\[Nu]rootminall[[i]];*)
				...
				exptemp1=SetPrecision[exptemp/.{\[Mu]n->\[Mu]i}/.{\[Omega]nr->wrpoint,\[Omega]ni->wipoint}/.{\[Nu]n->r\[Nu]+I i\[Nu]}/.\[Nu]root//LogicalExpand,setprec];
				Coeff=NSolve[exptemp1,{\[Lambda],c1,c2},WorkingPrecision->setprec-2][[l]];				
				FrobOfr[rt_]:=SetPrecision[Frob[x]//.Coeff//.{x->(rt M-rp[a])/(rp[a]-rm[a])}/.{\[Mu]n->\[Mu]i,\[Omega]nr->wrpoint,\[Omega]ni->wipoint}//Simplify,setprec];
				R0=SetPrecision[Limit[FrobOfr[rt],rt->rtstart[a]],setprec];
				dR0=SetPrecision[Limit[D[FrobOfr[rt],rt],rt->rtstart[a]],setprec];
			
				solR=NDSolve[{(DiffRadrt@R[rt]/.{\[Nu]n->r\[Nu]+I i\[Nu]}/.\[Nu]root/.{\[Mu]n->\[Mu]i,\[Omega]nr->wrpoint,\[Omega]ni->wipoint})==0,R[rtstart[a]]==R0,R'[rtstart[a]]==dR0},R,{rt,rtstart[a],rstop[\[Mu]i]},Method->{"StiffnessSwitching"},WorkingPrecision->setprec-2,MaxSteps->maxstep,AccuracyGoal->5]//First;

			         solRIterationI[[bb]]=solR;
				\[Nu]rootIterationI[[bb]]=\[Nu]root;
				\[Omega]IterationI[[bb]]=wipoint;
				,{bb,1,\[Omega]mesh}];
			logAbsRrstopI=Table[SetPrecision[Log[Abs[R[rt]/.solRIterationI[[kk]]]^2]/.{rt->rstop[\[Mu]i]},setprec],{kk,1,\[Omega]mesh}];
			minlogAbsRrstopI=Min[logAbsRrstopI];
			minposI=Position[logAbsRrstopI,minlogAbsRrstopI][[1]];
			Print[minposI];
			\[Omega]minI=\[Omega]IterationI[[minposI]]//First;
			
			(*Fixing the mesh boundary for the subsequent iteration*)
			If[minposI[[1]]==1||minposI[[1]]==\[Omega]mesh,(y=0;iiboundary=1.1 iidelta[i]),iiboundary=Abs[Log10[\[Omega]irange[[1]]]-Log10[\[Omega]irange[[3]]]]];

			(*Defining the new search interval for the next iteration; note, in this case the real search interval is symmetric around the new minimum*)
			Clear[\[Omega]irange];
			winti=SetPrecision[\[Omega]minI,setprec];					
			count=count+1;
			ltemp=l;
			If[count>8&&minposI=={1},l=Select[{1,2},#!=ltemp&]//First;branch=l;Print[l];count=0;winti=Im[\[Omega]minall[[i]]],{}];
			\[Omega]irange=SetPrecision[10^Range[Log10[winti]-iiboundary,Log10[winti]+iiboundary,(2iiboundary)/\[Omega]mesh],setprec];	
			If[count>100,count1=count;Break[];Print["Count: 100"],{}];
			];
			\[Nu]transfer=\[Nu]roottemp[[1]]+I \[Nu]roottemp[[2]];
			\[Omega]transfer=\[Omega]minR+I winti;	

Print["Simplex code..."];
(*translation into simplex code*)
iiinitial2=10^(Log10[Im[\[Omega]transfer]]-1);
...
While[
(*termination test:*)
testnorm>10^(Log10[Im[wint]]-preccutoff),
...
count=count+1;
If[count>10000,Print["Terminated simplex loop"];Break[],{}];
];
Print[testnorm];
	...
	branchtemp=branch;
	If[Abs[Log10[Im[\[Omega]minall[[i]]]]-Log10[Im[\[Omega]minall[[i+1]]]]]>4,branch=Select[{1,2},#!=branchtemp&]//First;Print[branch];count=0;\[Omega]minall[[i+1]]=\[Omega]minall[[i]],{}];
	Print[printw];
If[count>10000,Print["Terminated simplex loop: No Break[]!!!"],{}];
If[count1>100,Break[],{}];
If[aa>500,Print["Terminated loop 3"];Break[];,{}];
...
b=Table[Symbol["QNMmode`b"<>ToString[i]],{i,0,kmax}];
...
Sfunc[m_,\[Theta]_,\[Eta]_]:=Sum[Symbol["QNMmode`b"<>ToString[kp]]Y[Abs[m]+2kp+\[Eta],m,\[Theta]],{kp,0,kmax}];
...
linsys=Table[mattemp[[l]]==0,{l,1,kmax+1}]//.{b0->10};
solvar=Table[Symbol["QNMmode`b"<>ToString[i]],{i,0,kmax}];
AnglCoeff=Solve[linsys,solvar]//.{b0->10}//Flatten;
AnglCoeffList[[n]]=AnglCoeff;
Splot[\[Theta]_]:=Sfunc[m,\[Theta],\[Eta]]//.AnglCoeff//.{b0->10}//Simplify;
...
Export[ToString["/home/nsiemonsen/proca_superrad/m2_modes/m"<>ToString[m]<>ToString["n"]<>ToString[nh]<>ToString["_a"]<>ToString[spinstring]<>ToString["_S"]<>ToString[If[Sin<0,"m","p"]]<>ToString[Abs[Sin]]<>ToString["_AccGoal5_"]<>ToString[If[direc<1,"m","p"]]<>ToString["_HPe.mx"]],modedataoutput];
