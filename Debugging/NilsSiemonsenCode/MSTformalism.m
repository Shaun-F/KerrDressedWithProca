...
Almguess=SpinWeightedSpheroidalEigenvalue[-2,lin,min,c]+2min c-c^2;
...
gsol[i]=SetPrecision[If[Re[\[Nu]guess]<=1.01(lin-0.5),
	{With[{\[Nu]ri=If[lin==2,-0.5,lin-0.5]},
	{\[Nu]i->Abs[Re[\[Nu]i/.FindRoot[Re[gfunc[\[Nu]ri,\[Nu]i]]==0,{\[Nu]i,Im[\[Nu]guess]+1},MaxIterations->200]]]}],
	\[Nu]r->lin-0.5}//Flatten,
	
	{With[{\[Nu]ii=0},
		FindRoot[Re[gfunc[\[Nu]r,\[Nu]ii]]==0,{\[Nu]r,Re[\[Nu]guess]},MaxIterations->200]],
		\[Nu]i->0}//Flatten
										          ],20];
...
ufullBHPT[w_]:=
	prec@If[
		Im[RenormalizedAngularMomentum[-2,lin,min,\[Chi],2 SetPrecision[w,64],Method->"Monodromy"]]==0,
			RenormalizedAngularMomentum[-2,lin,min,\[Chi],2 SetPrecision[w,64],Method->"Monodromy"],
			lin-1/2+I Im[RenormalizedAngularMomentum[-2,lin,min,\[Chi],2 SetPrecision[w,64],Method->"Monodromy"]]
			];
]
...
Frob[x_]:=x^(-I \[Kappa]) (1+Sum[Symbol["c"<>ToString[n]]x^n,{n,1,2}]);
