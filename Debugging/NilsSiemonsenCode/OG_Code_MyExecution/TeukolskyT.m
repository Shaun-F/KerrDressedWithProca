...
contextSwitch = {TeuInterEnv`t->t, TeuInterEnv`r->r, TeuInterEnv`\[Theta]->\[Theta],TeuInterEnv`\[Phi]->\[Phi], Global`t->t, Global`r->r, Global`\[Theta]->\[Theta], Global`\[Phi]->\[Phi]};
...
Tdd=Import[Global`$root<>"Expressions/Tdd.mx"]//.coordinates//.{Global`\[Chi]->PrivateTeukolsky`\[Chi], Global`\[Mu]->PrivateTeukolsky`\[Mu]}/.contextSwitch;
...
With[{precval = TeuInterEnv`$precValue},
prec=SetPrecision[#,precval]&;
];
adowntemp=AdownNorm1//.{TeuInterEnv`t->t,TeuInterEnv`r->r,TeuInterEnv`\[Theta]->\[Theta],TeuInterEnv`\[Phi]->\[Phi]};
AdownInT[tt_,rr_,\[Theta]\[Theta]_,\[Phi]\[Phi]_]:=adowntemp//.{t->tt,r->rr,\[Theta]->\[Theta]\[Theta],\[Phi]->\[Phi]\[Phi]};
Aexpl=Table[ToExpression["Global`A"<>ToString[j]]->AdownInT[t,r,\[Theta],\[Phi]][[j+1]],{j,0,3}]//Flatten;
dAexpl=Table[{ToExpression["Global`D"<>ToString[i]<>ToString["A"]<>ToString[k]]->D[AdownInT[t,r,\[Theta],\[Phi]][[k+1]],{t,r,\[Theta],\[Phi]}[[i+1]]]},{i,0,3},{k,0,3}]//Flatten//Simplify;
Tddtemp=(Tdd//.PrivateTeukolsky`coordinates/.Aexpl/.dAexpl//.{\[Chi]->spin,\[Mu]->Procamass, PrivateTeukolsky`\[Chi]->\[Chi], PrivateTeukolsky`\[Mu]->\[Mu]});
Tdowndown={tt,rr,\[Theta]\[Theta],\[Phi]\[Phi]}|->Tddtemp/.{t->tt,r->rr,\[Theta]->\[Theta]\[Theta],\[Phi]->\[Phi]\[Phi], PrivateTeukolsky`t->tt, PrivateTeukolsky`r->rr, PrivateTeukolsky`\[Theta]->\[Theta]\[Theta], PrivateTeukolsky`\[Phi]->\[Phi]\[Phi]}//Evaluate;
];
...
Export[Global`$root<>"Expressions/TeuTintegrand.mx",TeukolskyTintegrand1];
TeukolskyTintegrand=Import[Global`$root<>"Expressions/TeuTintegrand.mx"];
...
tnnexpltemp=(nKtemp//.coordinates) . ((Tfielddowndown/.{TeuInterEnv`t->t,TeuInterEnv`r->r,TeuInterEnv`\[Theta]->\[Theta],TeuInterEnv`\[Phi]->\[Phi]}) . (nKtemp//.coordinates));
TnnExplNOE[tt_,rr_,\[Theta]\[Theta]_,\[Phi]\[Phi]_]:=(tnnexpltemp)/.{\[Chi]->spin,\[Mu]->Procamass}//.coordinates/.{t->tt,r->rr,\[Theta]->\[Theta]\[Theta],\[Phi]->\[Phi]\[Phi]};
tmnexpltemp=(nKtemp//.coordinates) . ((Tfielddowndown/.{TeuInterEnv`t->t,TeuInterEnv`r->r,TeuInterEnv`\[Theta]->\[Theta],TeuInterEnv`\[Phi]->\[Phi]}) . (mbarKtemp//.coordinates));
TmnExplNOE[tt_,rr_,\[Theta]\[Theta]_,\[Phi]\[Phi]_]:=(tmnexpltemp)/.{\[Chi]->spin,\[Mu]->Procamass}//.coordinates/.{t->tt,r->rr,\[Theta]->\[Theta]\[Theta],\[Phi]->\[Phi]\[Phi]};
tmmexpltemp=(mbarKtemp//.coordinates) . ((Tfielddowndown/.{TeuInterEnv`t->t,TeuInterEnv`r->r,TeuInterEnv`\[Theta]->\[Theta],TeuInterEnv`\[Phi]->\[Phi]}) . (mbarKtemp//.coordinates));
TmmExplNOE[tt_,rr_,\[Theta]\[Theta]_,\[Phi]\[Phi]_]:=(tmmexpltemp)/.{\[Chi]->spin,\[Mu]->Procamass}//.coordinates/.{t->tt,r->rr,\[Theta]->\[Theta]\[Theta],\[Phi]->\[Phi]\[Phi]};
TnnExpl = Global`OptimizedFunction[{t,r,\[Theta],\[Phi]},tnnexpltemp/.{\[Chi]->spin,\[Mu]->Procamass}//.coordinates//Evaluate];
...
tnntemp=ParallelTable[(-1)^mTeu 2\[Pi] FourierCoefficient[FourierTransform[TnnExpl[t,rrange[[j]],\[Theta]range[[k]],\[Phi]+\[Pi]]//TrigReduce,t,\[Omega]T]/.{DiracDelta[a_]:>1},\[Phi],mTeu],{j,1,Length[rrange]},{k,1,Length[\[Theta]range]}];
...
tmntemp=ParallelTable[(-1)^mTeu 2\[Pi] FourierCoefficient[FourierTransform[TmnExpl[t,rrange[[j]],\[Theta]range[[k]],\[Phi]+\[Pi]]//TrigReduce,t,\[Omega]T]/.{DiracDelta[a_]:>1},\[Phi],mTeu],{j,1,Length[rrange]},{k,1,Length[\[Theta]range]}];
...
tmmtemp=ParallelTable[(-1)^mTeu 2\[Pi] FourierCoefficient[FourierTransform[TmmExpl[t,rrange[[j]],\[Theta]range[[k]],\[Phi]+\[Pi]]//TrigReduce,t,\[Omega]T]/.{DiracDelta[a_]:>1},\[Phi],mTeu],{j,1,Length[rrange]},{k,1,Length[\[Theta]range]}];
...
TeukolskyTintegrandExpl=Global`OptimizedFunction[{r,\[Theta]},TTtemp//Evaluate];
...
Monitor[Do[
	...
	SWSHS[lTeu] = {\[Theta]}|->Evaluate[Sbar[\[Theta]]];
