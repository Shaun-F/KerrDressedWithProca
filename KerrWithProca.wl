...
CacheResults[assoc_, filename_]:=(Export[NotebookDirectory[]<>ToString[filename]<>".mx", assoc]);
assocToString[assoc_]:=StringReplace[StringJoin@@Table[ToString[(assoc//Keys)[[i]], InputForm]<>ToString[(assoc//Values)[[i]], InputForm], {i, 1, Length[assoc//Keys]}], {"/"-> "_", "\""-> ""}];
R2toC[vec_]:=vec[[1]]+I*vec[[2]];
IterabletoList[iter_]:=Table[iter[[i]], {i,1,iter//Length}];
DisjunctiontoList[disj_]:=Map[#[[2]]&,IterabletoList[disj]];
SymbolQ[sym_]:=MatchQ[Head@sym, _Symbol]
EmptyQ[expr_?ListQ]:=If[Length[expr]==0, True, False];
ToPrecision[parameters_][expr_]:=SetPrecision[expr, parameters["precision"]];
TotalQuantum[orbital_,overtone_]:=(overtone+orbital+1);
InsertValues[parameters_][expr_]:=expr/.{\[Mu]N->parameters["\[Mu]Nv"], m->parameters["m"],\[Eta]->parameters["\[Eta]"], n->parameters["n"], l->parameters["l"], s->parameters["s"], \[Chi]->parameters["\[Chi]"]};
GetIngoingSolution[frobsol_]:=Block[{inx=MapIndexed[If[Re[#1[[1]]]<=0,#2, Unevaluated[Sequence[]]]&,frobsol[[All,All,2]]]//First},frobsol[[inx]]];
JfromLS[l_,s_]:=Abs[l+s]//Rationalize[#,0]&;
QValue[\[Mu]_,\[Omega]_]:=Sqrt[\[Mu]^2-\[Omega]^2]
...
QApprox[\[Mu]_, Q1_, Q2_, \[Mu]1_,\[Mu]2_]:=(Q2-Q1)/(\[Mu]2 - \[Mu]1)*\[Mu] + (Q1*\[Mu]2 - Q2*\[Mu]1)/(\[Mu]2 - \[Mu]1)
calcEndingX[parameters_]:=4*10*(parameters["m"] + parameters["n"])/parameters["\[Mu]Nv"]^2
calcOrbital[parameters_]:=parameters["m"]+parameters["s"]
...
getResults[absolDir_?StringQ, parameterSet:(_?ListQ|Null):Null] :=
	...
	If[ListQ@parameterSet,
		If[Length@Dimensions@parameterSet==1,
			ParameterSetIndicies = StringContainsQ[ToString[parameterSet[[1]]]<>ToString[parameterSet[[2]]]]/@filenames;
			Return[Import/@Pick[filenames,ParameterSetIndicies], Module];
		];
	If[Length@Dimensions@parameterSet>1,
			ParameterSetIndicies = Table[StringContainsQ[ToString[parameterSet[[i]][[1]]]<>ToString[parameterSet[[i]][[2]]]]/@filenames, {i,1, Length@parameterSet}];
			ParameterSetIndiciesAll = Table[AllTrue[ParameterSetIndicies[[All,i]], TrueQ], {i, 1, Length@First@ParameterSetIndicies}];
			Return[Import/@Pick[filenames,ParameterSetIndiciesAll], Module];
		];
	];
];
...
(*Angular functions*)
qth[a_,\[Nu]_,\[Theta]_]:= 1-a^2*\[Nu]^2*Cos[\[Theta]]^2;
Kth[m_,a_,\[Omega]_,\[Theta]_]:= m-a*\[Omega]*Sin[\[Theta]]^2;
\[Sigma][a_,m_,\[Omega]_,\[Nu]_]:= a*\[Nu]^2 (m-a*\[Omega]) + \[Omega];
\[CapitalLambda][a_,m_,\[Omega]_,\[Nu]_,\[Mu]_] := \[Mu]^2/\[Nu]^2-\[Sigma][a,m,\[Omega],\[Nu]]/\[Nu]+2* a * \[Omega]*m - a^2*\[Omega]^2;
\[Gamma][\[Omega]_,\[Mu]_]:= \[Omega]^2-\[Mu]^2;
diffeqang[s_,m_,a_,\[Omega]_,\[Nu]_]:= Collect[qth[a,\[Nu],\[Theta]]/Sin[\[Theta]]*D[Sin[\[Theta]]/qth[a,\[Nu],\[Theta]]*D[s,\[Theta]],\[Theta]] - qth(Kth[m,a,\[Omega],\[Theta]]^2/(qth[a,\[Nu],\[Theta]]*Sin[\[Theta]]^2)+(2-qth[a,\[Nu],\[Theta]])/qth[a,\[Nu],\[Theta]]^2*\[Sigma][a,m,\[Omega],\[Nu]]/\[Nu]-\[Mu]^2/\[Nu]^2)s, {s, Derivative[1][s], s^\[Prime]\[Prime]}];
spherharmons = Flatten@Table[Row[{{l,m},Spacer[20],SphericalHarmonicY[l,m,\[Theta],0]}], {l,0,4},{m,-l,l}]//MatrixForm;
Off[ClebschGordan::tri];
Off[ClebschGordan::phy];
dl[l_]:=2*l+1;
sym[l1_,l2_,l3_,m_]:=(-1)^m*Sqrt[dl[l1]*dl[l2]*dl[l3]/(4*\[Pi])]*ThreeJSymbol[{l1,0},{l2,0},{l3,0}]*ThreeJSymbol[{l1,-m},{l2,0},{l3,m}];
all[l_,lp_,m_]:=4/3 Sqrt[ \[Pi]/5]sym[l,2,lp,m] + Sqrt[4*\[Pi]]/3*sym[l,0,lp,m];
bll[l_,lp_,m_]:= 2*Sqrt[\[Pi]]/5*sym[l,0,lp,m] + 8*Sqrt[\[Pi]/5]/7 sym[l,2,lp,m] + 16*Sqrt[\[Pi]]/105*sym[l,4,lp,m];
dll[l_,lp_,m_]:=Sqrt[4 \[Pi]/3](lp*Sqrt[(lp+1)^2-m^2]/Sqrt[(2lp+1)(2lp+3)]*sym[l,1,lp+1,m] - (lp+1) Sqrt[lp^2-m^2]/Sqrt[(2lp+1)(2lp-1)]*sym[l,1,lp-1,m]);
Mmatrixdef = Function[{l,lp,m,a,\[Omega],\[Nu],\[Mu]}, (\[CapitalLambda][a,m,\[Omega],\[Nu],\[Mu]] - lp*(lp+1))*KroneckerDelta[l,lp]  + (-\[Nu]^2*\[CapitalLambda][a,m,\[Omega],\[Nu],\[Mu]] + \[Nu]^2*lp*(lp+1) - 2*\[Sigma][a,m,\[Omega],\[Nu]]*\[Nu]+\[Gamma][\[Omega],\[Mu]])a^2*all[l,lp,m] - 2*a^2*\[Nu]^2*dll[l,lp,m] - \[Gamma][\[Omega],\[Mu]]*\[Nu]^2*a^4*bll[l,lp,m]];

Mmatrix = Function[{KMax,eta,m,a,\[Omega],\[Nu],\[Mu]}, Table[Mmatrixdef[m+2*k+eta,m+2*kp+eta,m,a,\[Omega],\[Nu],\[Mu]], {k,0,KMax},{kp,0,KMax}]];

Mmatrixdet[\[Omega]N_,\[Nu]N_, parameters_]:=
Block[{
Mmatrixdettemp= Evaluate[\[Nu]N^parameters["KMax"]*Mmatrix[parameters["KMax"],parameters["\[Eta]"],parameters["m"],parameters["\[Chi]"]*M,\[Omega]N/M,\[Nu]N/M,parameters["\[Mu]Nv"]/M]//Det//Refine//Collect[#,M]&//Collect[#,\[Nu]N]&//Refine]
},
Evaluate@Block[{eta=parameters["\[Eta]"],m=parameters["m"],\[Chi]=parameters["\[Chi]"],\[Mu]N=parameters["\[Mu]Nv"]},Evaluate@Mmatrixdettemp]
];

ansatz[b1_]:= {1, b1,0};

SolveAngularSystem[\[Omega]value_?NumberQ, \[Nu]value_?NumberQ, parameters_]:=
	Block[{bcoeffs = Table[C[i], {i,0,parameters["KMax"]}], \[Eta]=parameters["\[Eta]"], m=parameters["m"]},
		NumMatrix = \[Nu]N^-7*Mmatrix[parameters["KMax"],\[Eta],m,\[Chi]*M,\[Omega]N/M,\[Nu]N/M,\[Mu]N/M]//N;
		equalto = Table[0, {i,0, parameters["KMax"]}];
		System = (NumMatrix . bcoeffs==equalto)/.{\[Omega]N->\[Omega]value,\[Nu]N-> \[Nu]value, \[Mu]N-> parameters["\[Mu]Nv"], m-> parameters["m"], \[Eta]-> parameters["\[Eta]"], \[Chi]-> parameters["\[Chi]"]};
		Return[Flatten@{C[0]->1, Solve[System/.C[0]->1, bcoeffs[[2;;parameters["KMax"]+1]]]}];
];


(*Radial functions*)
qr[\[Nu]_,r_]:= 1+\[Nu]^2*r^2;
\[Sigma][a_,m_,\[Omega]_,\[Nu]_]:= a*\[Nu]^2 (m-a*\[Omega]) + \[Omega];
Kr[a_,m_,r_]:= -a*m + (a^2+r^2)\[Omega];
rplus = (rs + Sqrt[rs^2-4*a^2])/2;
rminus = (rs - Sqrt[rs^2-4*a^2])/2;
\[CapitalDelta] = (r-rplus)(r-rminus);
diffeqrad = D[\[CapitalDelta]/qr*D[R[r],r],r] + (Kr[a,m,r]^2/(qr*\[CapitalDelta])+(2-qr)/qr^2*\[Sigma][a,m,\[Omega],\[Nu]]/\[Nu]-\[Mu]^2/\[Nu]^2)R[r]==0;
diffeqradOp = (D[\[CapitalDelta]/qr*D[#,r],r] + (Kr[a,m,r]^2/(qr*\[CapitalDelta])+(2-qr)/qr^2*\[Sigma][a,m,\[Omega],\[Nu]]/\[Nu]-\[Mu]^2/\[Nu]^2)#)&;

\[CapitalKappa][\[Omega]_,\[Chi]_,m_]:=(\[Omega]*rplusN[\[Chi]]*(rplusN[\[Chi]]+rminusN[\[Chi]]) - \[Chi]*m)/(rplusN[\[Chi]]-rminusN[\[Chi]]);
rplusN[\[Chi]_]:=1 + (1-\[Chi]^2)^(1/2);
rminusN[\[Chi]_]:=1 - (1-\[Chi]^2)^(1/2);
...
\[Sigma]N = \[Sigma][a,m,\[Omega],\[Nu]]*M//Expand//Rationalize;
KrN = Kr[a,m,rN*M]/M//Rationalize//Simplify;
qrNX = qrN/.rN->xN + rplusN[\[Chi]]//Rationalize//FullSimplify;
KrNX = KrN/.rN->xN + rplusN[\[Chi]]//Rationalize//FullSimplify;
\[Sigma]NX = \[Sigma]N/.rN->xN+rplusN[\[Chi]]//Rationalize//FullSimplify;
\[CapitalDelta]NX = \[CapitalDelta]N/.rN->xN+rplusN[\[Chi]]//Rationalize//FullSimplify;
P1 = (2*rN - rsN)/(rN-rminusN[\[Chi]])//Rationalize;
P2 = -(2*rN*\[Nu]N^2)/qrN;
Q1 = KrN^2/(rN-rminusN[\[Chi]])^2;
Q2 = qrN/(rN-rminusN[\[Chi]])*((2-qrN)/qrN^2*\[Sigma]N/\[Nu]N-\[Mu]N^2/\[Nu]N^2)//Rationalize;
...
Evaluate[Import[notebookPath<>"FrobeniusSystem.mx"][\[Kappa],c1,c2,\[Omega]Nt, \[Nu]Nt, parameters["m"], parameters["\[Chi]"], parameters["\[Mu]Nv"]]]
];
...
omegaNRNonRel[parameters_]:=Block[{\[Mu]N=parameters["\[Mu]Nv"],m=parameters["m"], l=parameters["l"],n=parameters["n"],s=parameters["s"]},\[Mu]N*(1 - \[Mu]N^2/(2*TotalQuantum[l,n]^2))//Rationalize[#,0]&];

gfuncN[j_,parameters_]:=Block[{\[Chi]=parameters["\[Chi]"],\[Mu]N=parameters["\[Mu]Nv"],m=parameters["m"], l=parameters["l"],n=parameters["n"],s=parameters["s"]},
...
With[{NN = n+l+1},
If[NN>=0,
 ((2^(2l+2j+1)*(NN+l)!)/(NN^(2l+4)*(NN-l-1)!))(l!/((l+j)!(l+j+1)!))^2*(1 + (2(1+l-j)(1-l+j))/(l+j))^2//Rationalize,
Print["Error: Principal quantum number less than zero!"];Abort[]]];

omegaNINonRel[parameters_]:=
Block[{\[Chi]=parameters["\[Chi]"],\[Mu]N=parameters["\[Mu]Nv"],m=parameters["m"],l=parameters["l"],n=parameters["n"],s=parameters["s"]},
Block[{j=Abs[m]},2*rplusN[\[Chi]]*Ccoeff[n,l,j]*gfuncN[j,parameters]*(m*KerrSurfaceRotationN[\[Chi]] - omegaNRNonRel[parameters])*\[Mu]N^(2l+2j+5)]
];

nuNNonRel[parameters_]:=
Block[{\[Chi]=parameters["\[Chi]"], \[Mu]N=parameters["\[Mu]Nv"],m=parameters["m"],l=parameters["l"],n=parameters["n"], PolarizationSigned=parameters["s"]},
Block[{\[Omega]NNonRel = omegaNRNonRel[parameters] + omegaNINonRel[parameters]*I},
Evaluate@Switch[PolarizationSigned,
-1, (-\[Omega]NNonRel)/(m-\[Chi]*\[Omega]NNonRel),
0,(1/(2*\[Chi]))(m+1-\[Chi]*\[Omega]NNonRel+\[Sqrt]((\[Chi]*\[Omega]NNonRel - m - 1)^2+4*\[Chi]*\[Omega]NNonRel)),
1,Evaluate@Block[{X=\[Chi],\[Nu]},Roots[X*\[Nu]^3 (m - X*\[Omega]NNonRel)-\[Nu]^2 ((m+1)(m+2) - X*\[Omega]NNonRel(2m-X*\[Omega]NNonRel)) + \[Omega]NNonRel*\[Nu] + \[Omega]NNonRel^2==0,\[Nu]][[2]]]
...
Block[{\[Chi]=parameters["\[Chi]"],m=parameters["m"], PolarizationSigned=parameters["s"]},
Evaluate@Switch[PolarizationSigned,
-1, (-\[Omega]NNonRel)/(m-\[Chi]*\[Omega]NNonRel),
0,(1/(2*\[Chi]))(m+1-\[Chi]*\[Omega]NNonRel+\[Sqrt]((\[Chi]*\[Omega]NNonRel - m - 1)^2+4*\[Chi]*\[Omega]NNonRel)),
1,Evaluate@Block[{X=\[Chi],\[Nu]},Roots[X*\[Nu]^3 (m - X*\[Omega]NNonRel)-\[Nu]^2 ((m+1)(m+2) - X*\[Omega]NNonRel(2m-X*\[Omega]NNonRel)) + \[Omega]NNonRel*\[Nu] + \[Omega]NNonRel^2==0,\[Nu]][[2]]]
...
getNuValue[\[Omega]N_?NumberQ, parameters_, ClosestGuess_?NumberQ]:=
	Block[{eta=parameters["\[Eta]"],m=parameters["m"],\[Chi]=parameters["\[Chi]"],\[Mu]N=parameters["\[Mu]Nv"]},
		Module[{thedet = Function[{w,\[Nu]N},Evaluate@Mmatrixdet[w,\[Nu]N,parameters]]},
				FindRoot[thedet[\[Omega]N,\[Nu]N]// ToPrecision[parameters],{\[Nu]N,ClosestGuess}, WorkingPrecision->parameters["precision"], MaxIterations->1000][[1]][[2]]//Rationalize[#,0]&//Return
		]
	];(*Min@Cases[%,Power[\[Nu]N,x_?NumberQ]\[RuleDelayed]x,-1]*)


functomin[parameters_?AssociationQ][\[Omega]realimaglist_?ListQ]:=
Block[{\[Omega]value=\[Omega]realimaglist[[1]]+I*\[Omega]realimaglist[[2]]},
Block[{nuroot = Evaluate@getNuValue[\[Omega]value, parameters,Evaluate@ ToPrecision[parameters]@nuNNonRel[parameters]]},
retval = getRadialSolution[\[Omega]value,nuroot,parameters][parameters["EndingX"]]//Abs;
(*Return large negative value if retval is 0. Cures errors raised by taking naive logarithm*)
If[retval==0, Return[-100], Return[Log10[retval]]];
]
];


RadialMinimize[\[Omega]Guess_?NumberQ,\[Nu]Guess_?NumberQ, OmegaBoundary_?ListQ, parameters_?AssociationQ, Watcher_?BooleanQ]:=
Module[{currval = {0,0},CurrentIteration = 0},
RMMessengerGenerator = Function[{X,counter}, "Current Value: "<>ToString[X, InputForm]<>"\n Current Step: "<>ToString[counter, InputForm]];
...
result = NMinimize[{Hold@functomin[parameters][{\[Xi],\[Psi]}], \[Psi]>0}, {\[Xi],\[Psi]}, 
Method->{"NelderMead", "PostProcess"->False, "InitialPoints"-> InitialSimplex}, 
WorkingPrecision->parameters["OptimizationAccuracy"], 
EvaluationMonitor:>(
RMMessenger =RMMessengerGenerator[\[Xi]+I*\[Psi], CurrentIteration];
CurrentIteration++;), 
MaxIterations->parameters["MaxInterationsMinimization"]
][[2]];
\[Omega]Result = (\[Xi]+I*\[Psi])/.result;
\[Nu]Result = getNuValue[\[Omega]Result, parameters, nuNNonRel[\[Omega]Result, parameters]];
